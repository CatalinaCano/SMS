  ...
  */
  ...
  buscarPorOrden() {
     (<HTMLInputElement> document.getElementById('txtNumeroOrden')).disabled = true;
      ...
      this._ordenesService.obtenerDatosOrden(this.numero_orden.trim())
                          .subscribe( res => {
  ...
  base64Observable.subscribe(res =>
    this._subirArchivo.subirArchivo(this.tipo_envio, this.cod_proveedor, this.nombreArchivo, res)
                      .subscribe(
                        res => {  swal('Registro exitoso...', 'El Archivo se almaceno con éxito', 'success');
                                  this.limpiar();
                       }, err => {
                        swal('Error!!', 'Se ha producido un error al subir el archivo', 'error');
                        this.limpiar();
                       }
                       ));
  }
